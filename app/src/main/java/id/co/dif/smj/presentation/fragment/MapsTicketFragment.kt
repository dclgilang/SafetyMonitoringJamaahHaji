...
import id.co.dif.smj.databinding.FragmentMapsTicketBinding
    ...
    private fun setupLocations(ticketDetails: TicketDetails?) {
        ticketDetails?.let { data ->
            if (!requireContext().isDeviceOnline()) {
                ...
                viewModel.getNearestTechnician(data.site_info?.siteId)
            }
        }
            ...
            if (it.status in StatusCode.SUCCESS) {
                ...
                ticketDetails?.let { data ->
                    ...
                    when (session?.emp_security) {
                        1 -> Unit
                        2 -> viewModel.getDetailProfile(data.tic_person_in_charge_id?.toInt())
